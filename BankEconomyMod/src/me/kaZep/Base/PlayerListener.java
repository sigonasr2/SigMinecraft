...
implements Listener
{
  ...
  public void onFurnaceGet(FurnaceExtractEvent e) {
	...
	public void onInventoryOpen(InventoryOpenEvent e) {
		...
		// ç¬†ï¿½ç¬†ï¿½(Player)e.getPlayer()).sendMessage(((Player)e.getPlayer()).getScoreboard().getPlayerTeam((OfflinePlayer)e.getPlayer()).getName());
		...
		if (event.getInventory().getType()==InventoryType.CRAFTING /*|| event.getInventory().getType()==InventoryType.CHEST*//*Buggy for some reason. We can't open chests in chests.*/) {
			...
			if (event.getInventory().getType()==InventoryType.CHEST && event.getInventory().getName().contains("Item Cube")) {
				...
				if (event.getInventory().getType()==InventoryType.BREWING) {
					...
					Bukkit.getLogger().info("Anvil Inventory CLICK");

					if (event.getSlotType() == SlotType.CONTAINER
							&& (event.getSlot() == INPUT || event.getSlot() == MATERIALS
							|| event.getSlot() == MAGIC || event.getSlot() == OUTPUT))
					{
						...
						if (event.getSlot() == OUTPUT && event.getCursor() != null && !matches(event.getCursor(), event.getCurrentItem())){
						...
						if (event.getInventory().getContents()[OUTPUT] != null
							...
							if (event.getInventory().getContents()[LEVELS].getAmount() > Bukkit.getPlayer(event.getWhoClicked().getName()).getLevel()) {
								...
								p.sendMessage("You don't have enough experience to do that.");
								event.setCancelled(true);
							} else {
								if (event.getCursor() != null) {
						...
						if (!valid) {
							...
							Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(this.plugin, new updateInventoryTask(event.getInventory(), Bukkit.getPlayer(event.getWhoClicked().getName())));
						}
					} else {
						event.setCancelled(true);
					}
				...
				if ((lines[0].equalsIgnoreCase("[Bank]")) && (lines[1].equalsIgnoreCase("Withdraw"))) {
					...
					if ((lines[0].equalsIgnoreCase("[Bank]")) && (lines[1].equalsIgnoreCase("Check Balance"))) {
						...
						if ((lines[0].equalsIgnoreCase("[Bank]"))) {
							e.setCancelled(true);
							p.sendMessage("ï¿½ï½§cYou do not have permission to create [Bank] signs.");
						}
					}
		}
		/*
    if (p.hasPermission("bankeconomy.sign.create")) {
      if ((lines[0].equalsIgnoreCase("[BankEconomy]")) && (lines[1].equalsIgnoreCase("deposit"))) {
        ...
        if (!lines[2].matches("^[0-9]+$")) {
          ...
          e.setLine(2, " ");
          p.sendMessage("ï¿½ï½§2[BankEconomy]" + ChatColor.AQUA + " The amount of money is invalid. (line 3)");
        } else if (!lines[3].isEmpty()) {
          e.setLine(1, ChatColor.DARK_RED + "Error");
          e.setLine(2, " ");
          e.setLine(3, " ");
          p.sendMessage("ï¿½ï½§2[BankEconomy]" + ChatColor.AQUA + " The line 4 must be empty.");
        } else {
          e.setLine(1, ChatColor.YELLOW + "deposit");
          p.sendMessage("ï¿½ï½§2[BankEconomy]" + ChatColor.AQUA + " Sign created successfully!");
        }
        ...
        if ((!lines[2].isEmpty()) || (!lines[3].isEmpty())) {
          ...
          e.setLine(2, " ");
          e.setLine(3, " ");
          p.sendMessage("ï¿½ï½§2[BankEconomy]" + ChatColor.AQUA + " Lines 3 and 4 must be empty.");
        } else {
          e.setLine(0, ChatColor.DARK_GREEN + "[BankEconomy]");
        ...
        e.setLine(2, " ");
        e.setLine(3, " ");
        p.sendMessage("ï¿½ï½§2[BankEconomy]" + ChatColor.AQUA + " Invalid action.");
      }
    } else {
    	if ((lines[0].equalsIgnoreCase("[BankEconomy]"))) {
	      e.setCancelled(true);
	      p.sendMessage("ï¿½ï½§cYou do not have permission.");
    	}
    }*/
	}
    	 ...
    	 Bukkit.getWorld("world").spawnEntity(p.getEyeLocation().add(direction.getX(), direction.getY(), direction.getZ()), EntityType.SILVERFISH);
			...
			if ((e.getClickedBlock().getState() instanceof BrewingStand)) {
              ...
              if (value <= actHand) {
                ...
                this.plugin.saveAccountsConfig();
                if (value <= 1)
                  p.sendMessage("ï¿½ï½§2[BankEconomy]" + ChatColor.AQUA + " You added " + value + " " + currencySG + " to your bank account.");
                else {
                  p.sendMessage("ï¿½ï½§2[BankEconomy]" + ChatColor.AQUA + " You added " + value + " " + currencyPL + " to your bank account.");
                }
                Main.economy.withdrawPlayer(p.getName(), value);
              } else {
                p.sendMessage("ï¿½ï½§2[BankEconomy]" + ChatColor.AQUA + " Sorry, you can't deposit that amount of money.");
              }*/
              ...
              if (value <= actMon) {
                ...
                this.plugin.saveAccountsConfig();
                if (value <= 1)
                  p.sendMessage("ï¿½ï½§2[BankEconomy]" + ChatColor.AQUA + " You withdrawn " + value + " " + currencySG + " from your bank account.");
                else {
                  p.sendMessage("ï¿½ï½§2[BankEconomy]" + ChatColor.AQUA + " You withdrawn " + value + " " + currencyPL + " from your bank account.");
                }
                Main.economy.depositPlayer(p.getName(), value);
              } else {
                p.sendMessage("ï¿½ï½§2[BankEconomy]" + ChatColor.AQUA + " Sorry, you can't withdraw that amount of money.");
              }*/
...
class updateInventoryTask implements Runnable {
    Inventory anvilInv = null;
    Player player = null;
    public updateInventoryTask(Inventory inv, Player pl){
        anvilInv = inv;
        player = pl;
    }
    @Override
    public void run() {
		Bukkit.getLogger().info("Runnable task run() called");
    	...
    	// If materials stacked past 5, reduce to 5 and drop the rest on the ground.
		if (anvilInv.getItem(MATERIALS).getAmount() > 5) {
			Bukkit.getLogger().info("Materials exceed 5 stack. Dropping " + (anvilInv.getItem(MATERIALS).getAmount() - 5) + " of " + anvilInv.getItem(MATERIALS).getType());
			player.getWorld().dropItemNaturally(player.getLocation(), new ItemStack(anvilInv.getItem(MATERIALS).getType(), anvilInv.getItem(MATERIALS).getAmount() - 5));
			anvilInv.getItem(MATERIALS).setAmount(5);
		}
